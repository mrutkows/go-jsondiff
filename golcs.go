...
// see: https://github.com/yudai/golcs (original impl.)
// see: https://www.geeksforgeeks.org/longest-common-subsequence-of-two-arrays-out-of-which-one-array-consists-of-distinct-elements-only/
// TODO: verify how "naive" this impl. is and seek to improve
type Lcs interface {
...
func (lcs *lcs) TableContext(ctx context.Context) (table [][]int, err error) {
	...
	for y := 1; y < sizeY; y++ {
		...
		for x := 1; x < sizeX; x++ {
			...
			table[x][y] = maxInt(table[x-1][y-1]+increment, table[x-1][y], table[x][y-1])
		}
	}
